...
def main() -> None:
    ...
    parser.add_argument(
        '--model',
        type=str,
        default='model/mvanet_1024x1024.onnx',
    )
    ...
    while True:
        ...
        cv2.imshow('MVANet : Input', frame)
        cv2.imshow('MVANet : Output', mask)
        cv2.imshow('MVANet : Debug', debug_image)
        key = cv2.waitKey(1)
        if key == 27:  # ESC
            break
...
if __name__ == '__main__':
    if not os.path.exists('./model/mvanet_1024x1024.onnx'):
        url = 'https://github.com/Kazuhito00/MVANet-ONNX-Sample/releases/download/v0.0.1/mvanet_1024x1024.onnx'
        save_path = './model/mvanet_1024x1024.onnx'
